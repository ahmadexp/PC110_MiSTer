...
The app can import firmware and disk-image files that you select through the iOS document picker. Those files are validated and copied into the app's private on-device storage for emulation. They are not uploaded by PC 110. Replaced files receive a local backup so an import can be reversed.
